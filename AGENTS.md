...
- `wiki_*.txt`: Cached Wikipedia training data (generated, committed for licensing reasons)
- `create_language_model.py`: **DO NOT RUN AUTOMATICALLY** - Retrains bigram models from Wikipedia
...
**Current Status**: Not yet working reliably (as of 2025)
...
4. Generates/updates `lang<language>model.py` in `chardet/`
5. Commit both `wiki_*.txt` and `lang*model.py` files
6. Test thoroughly with existing and new test files

**DO NOT**:
- Run `create_language_model.py` automatically without explicit request
...
- chardet is intentionally slower than alternatives
